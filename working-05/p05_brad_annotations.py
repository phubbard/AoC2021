...
def draw_horizontal(rcmd, data2d):
    ...
    snoop_start(f"horizontal {col1=} {row1=} {col2=} {row2=}")
...
def draw_vertical(rcmd, data2d):
    ...
    snoop_start(f"vertical {col1=} {row1=} {col2=} {row2=}")
    for row in range(begin, end + 1):
        data2d[row][col1] += 1
    return data2d


def snoop_start(string):
    print(f"SNOOP STARTING {string}")

def snoop(x, y):
    print(f"SNOOP NEXT {x}, {y}")
...
def draw_upright_diagonal(rcmd, data2d):
    ...
    snoop_start(f"upright_diagonal {col1=} {row1=} {col2=} {row2=}")
...
def draw_downright_diagonal(rcmd, data2d):
    ...
    snoop_start(f"downright_diagonal {col1=} {row1=} {col2=} {row2=}")
...
def draw_diagonal(rcmd, data2d):
    ...
    if row1 < row2 and col1 < col2:
        snoop_start(f"case one: Down and to the right")
        return draw_downright_diagonal(rcmd, data2d)
    if row1 > row2 and col1 < col2:
        snoop_start(f"case two: Up and to the right")
        return draw_upright_diagonal(rcmd, data2d)
    if row1 < row2 and col1 > col2:
        snoop_start(f"case three: down and to the left")
        return draw_upright_diagonal((col2, row2, col1, row1), data2d)
    if row1 > row2 and col1 > col2:
        snoop_start(f"case four: up and to the left")
        return draw_downright_diagonal((col2, row2, col1, row1), data2d)
    snoop_start(f"DIAGONAL FAIL {row1=} {row2=} {col1=} {col2=}")
    pass


def part_two(commands):
    num_rows = max(max(x[0], x[2]) for x in commands) + 2
    num_cols = max(max(x[1], x[3]) for x in commands) + 2
    dim = max(num_cols, num_rows)
    data = make_2d_array(dim, dim)
    print(f'{len(commands)=} {num_cols=} {num_rows=}')
    for command in commands:
        snoop_start(f"command {command=}")
